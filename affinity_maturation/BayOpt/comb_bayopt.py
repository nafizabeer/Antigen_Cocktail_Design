'''
minor edits in https://github.com/facebookresearch/bo_pr/blob/main/discrete_mixed_bo/run_one_replication.py
'''
...
def run_one_replication(
    ...
    if X_init is None:
        ...
        X = X_init.to(**tkwargs)
        if Y_init is None:
            Y_init = eval_problem(X, base_function=base_function)
        Y = Y_init.to(**tkwargs)
    ...
    if is_moo:
        ...
        if feas.any():
            temp_idx = Y[feas, 0].argmax()
            best_objs = Y[feas, 0][temp_idx].view(-1).cpu()
            opt_X.append(X[feas][temp_idx].cpu().data.numpy())    
    ...
    opt_Y.append(best_objs.cpu().data.numpy())
    num_queries.append(len(base_function.scorer.score_dict))
    ...
    for i in range(existing_iterations, iterations):
        ...
        print(
            ...
            f"queries: {len(base_function.scorer.score_dict)}"
        )
        # Fit the model.
        ...
        if is_moo:
            ...
            if feas.any():
                temp_idx = Y[feas, 0].argmax()
                best_obj = Y[feas, 0][temp_idx].view(-1).cpu()
                opt_X.append(X[feas][temp_idx].cpu().data.numpy())
            else:
        ...
        opt_Y.append(best_objs[-1].cpu().data.numpy())

        if opt_Y[-1]>opt_Y[-2]:
            num_queries.append(len(base_function.scorer.score_dict))
        else:
            num_queries.append(num_queries[-1])
        ...
        if save_frequency is not None and iterations % save_frequency == 0:
            ...
            if save_callback_2 is not None:
                save_callback_2({'score': np.hstack(opt_Y), 'best_member':opt_X})
    ...
    save_callback(output_dict)
    if save_callback_2 is not None:
        save_callback_2({'score': np.hstack(opt_Y), 'best_member':opt_X, 'num_queries': np.hstack(num_queries)})

    print(
        f"Finished {label}, seed {seed}, iteration {i}, "
        f"time: {time()-start_time}, final best obj: {best_objs[-1]}."
        f"queries: {len(base_function.scorer.score_dict)}"
    )
